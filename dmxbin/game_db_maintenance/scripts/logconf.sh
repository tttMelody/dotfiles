...
_writelog() {
    ...
    if [ $level -le $m_logLevel ]; then
        if [ ${m_logMode} -eq 1 ]; then
            echo -e "${m_logLevelArray[$level]}: `date '+%Y-%m-%d %H:%M:%S'` $m_logModuleName * $$ $@"  >> ${m_logFileName}
        else
            echo -e "${m_logLevelArray[$level]}: `date '+%Y-%m-%d %H:%M:%S'` $m_logModuleName * $$ $@" | tee -a $m_logFileName
        fi
    fi
    return 0;
}
